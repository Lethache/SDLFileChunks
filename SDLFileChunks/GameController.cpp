...
void GameController::RunGame()
{
	...
	r->ChangeDisplayMode(&r->GetResolutions()[5], true);// add a number to select the resolution you want to test from the list of available resolutions
    ...
    while (m_sdlEvent.type != SDL_EVENT_QUIT)
    {
        ...
        r->RenderTexture(
            sheet,
            sheet->Update(EN_AN_RUN, t->GetDeltaTime()),
            SDL_FRect{ 0, 150, 69 * 3, 44 * 3 }
        );
        ...
        std::string res =
            "Resolution: " +
            std::to_string(r->GetWindowSize().x) + "x" +
            std::to_string(r->GetWindowSize().y);

        font->Write(r->GetRenderer(), res.c_str(), SDL_Color{ 0, 0, 255 }, SDL_Point{ 0, 20 });

        std::string s =
            "Frame number: " +
            std::to_string(sheet->GetCurrentClip(EN_AN_RUN));
